...
CREATE PROCEDURE [dbo].[usp_Generate_Merge_For_Table]
	...
	@update_existing_rows	BIT = 0,	-- enable/disable UPDATE of rows
...
BEGIN
	...
	BEGIN
		IF (SELECT COLUMNPROPERTY( OBJECT_ID(QUOTENAME(@CurrSchema) + '.' + @CurrTable),SUBSTRING(@CurrColumnName,2,LEN(@CurrColumnName) - 2),'IsComputed')) = 1 OR @CurrColumnType IN ('timestamp','rowversion') AND @include_timestamp = 0
		BEGIN
			GOTO SKIP_COLUMN					
		END
	END
	...
	CASE
		...
		WHEN @CurrColumnType LIKE '%date%'
			THEN 
				'COALESCE('''''''' + RTRIM(CONVERT(varchar(max),' + @CurrColumnName + ',121))+'''''''',''NULL'')'
